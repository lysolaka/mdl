...
impl Playlist {
    /// Tag the playlist's converted files (searched for in `<indir>/<self.id>` or `<self.id>` if
    /// `indir` is `None`).
    ///
    /// Remember to convert the playlist album first (see [`Playlist::postprocess()`]).
    ///
        ...
        P2: AsRef<Path>,
    {
        ...
        log::info!("Finished tagging {}. The tagged files can be found in {}", self.title(), outdir.display());
        Ok(())
    }
}
